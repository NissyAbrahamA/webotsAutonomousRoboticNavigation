...
class RoadDetector:
    ...
    def detect_lanes(self, image):
        ...
        #This is added to handle the pedestian crossing which is also yellow in colour. when pedestrain crossing
        #dim the range to avoid steering away from centroid of the yellow line
        if moments["m00"] > 500000:   
            lower_yellow = np.array([20, 100, 100])
            upper_yellow = np.array([30, 130, 255])
            mask_yellow = cv2.inRange(hsv, lower_yellow, upper_yellow)
            mask_yellow = cv2.Canny(mask_yellow,100,200)
