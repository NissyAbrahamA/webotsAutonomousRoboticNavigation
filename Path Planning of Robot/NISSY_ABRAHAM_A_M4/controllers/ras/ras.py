...
The agent is a reactive navigation agent. The robot drives around randomly based on the input image,
taking right and left turns randomly at intersections and stops for STOP sign. When battery is low, identifies the shortest path 
to go to the destination to recharge. Here both image and GPS sensors are used to sense the environment and act accordingly.
Therefore it follows SENSE-ACT approach. (RAS Unit 3  Software organisation) Though there ExploreMode code is avialble, its never run in 
this misison and the graph generated already is reused
"""
class MyRobot(RASRobot,RoadDetector,ExploreGraph,FollowShortestPath):
    ...
    def detect_stop_sign(self, image):
        ...
        """
        # Use the pretrianed Yolov5 object detection model to detect objects in the image
    ...
    def run(self):
        ...
        """
        #vatiable initialisation
        low_battery = 0.40*self.time_to_live        
        destination = "(-210, -165)"
        car_md = None 
        prev_gps_cords = None
        stop_detected = False
        ...
        while self.tick():
            ...
            Refernced - https://stackoverflow.com/questions/59230049/find-the-shortest-distance-between-sets-of-gps-coordinates-python. No code snippet was copied directly
            """
            #If battery is low, then identify the shortest path in the graph to the destination
            if self.time_to_live < low_battery and self.Turn == False:  
                #print(self.Path)
                batterylow = True
                if self.Path == None:
                    ...
                    if graph.has_node(strsource):
                        ...
                        if self.Path !=None:
                            print('Battery Low - shortest path to destination identified')
            else:
                self.Path = None
            ...
            # Edge case - If battery low threshold is reached at a intersection, the direction of turn is set and so the identified 
            ...
            #As the image is scanned for every 10 ticks, the stop sign may be detected earlier, so to avoid stopping car too early, car is stopped after 10 ticks after detection of stop sign    
            ...
            elif steering_angle == self.RightTurn or steering_angle == self.LeftTurn:
